...
public:
  ...
  * @param filename
  */
  bool loadFromBinaryFile(const char* voc_version, const std::string &filename);
...
template<class TDescriptor, class F>
bool TemplatedVocabulary<TDescriptor, F>::loadFromBinaryFile(const char* voc_version, const std::string &filename) {
	fstream f;
	f.open(filename.c_str(), ios_base::in | ios::binary);
	unsigned int nb_nodes, size_node;
	char voc_version_stored[20];
	f.read(voc_version_stored, sizeof(char) * 20);
	cout << "voc_version stored is " << voc_version_stored << endl;
	cout << "cfg version is " << voc_version << endl;
	if (strcmp(voc_version_stored, voc_version) == 0)
	{
		cout << "Correct Version of VOC" << endl;
	}
	else
	{
		cout << "Warning: You have loaded wrong version of VOC" << endl;
	}
	...
	while (!f.eof()) {
		...
		if (m_nodes[nid].parent < 0 || m_nodes[nid].parent > nb_nodes)
		{
			break;
		}
		//m_nodes[nid].parent = *(const int*)buf;
		m_nodes[m_nodes[nid].parent].children.push_back(nid);
		//m_nodes[nid].descriptor = m_pBuf+DBoW2::FORB::L*(nid-1);// cv::Mat(1, F::L, CV_8U);
...
void TemplatedVocabulary<TDescriptor, F>::saveToBinaryFile(const std::string &filename) const {
	...
	f.write(voc_version, sizeof(char) * 20);
